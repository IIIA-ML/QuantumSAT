    ...
    def get_variables(self):
        ...
        for mapper in self.mappers: #mapper = [Q's, encoding's]
            for k, v in mapper[1].items():
                if v!=None and v not in variables:
                    variables.append(v)
...
class Bian(Joiner):
    ...
    def Join(self, mappers):
        ...
        #ARREGLAR AIXO I POSAR-HO PER A LA FINAL_Q!!!!!!!!!!!!!!!!
        variables = self.get_variables()
        for v in variables:
            repeated_variables = sorted([index for index, variable in final_encoding.items() if variable==v])
...
class SAT_variables(Joiner):
    def get_index_aux(self):
        ...
        for mapper in self.mappers: #mapper = [Q's, encoding's]
            ...
            #        index_aux += 1
            #        var_list.append(v)
        return index_aux+1 #Index necessary for the encoding of bian
