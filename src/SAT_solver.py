...
class Variable_encoding:
    def __init__(self, encoding=None, variables=None, variable_to_literal=None, literal_to_variable=None):
        self.variables = None
        self.variable_to_literal = None
        self.literal_to_variable = None
...
class SAT_solver:
    def __init__(self, instance=None, file_path=None, splitter=None, sorter=None, gadget=None, joiner=None, solver=None, token=None, qubit_level=False):
        self.clauses = None
        self.N = None
        self.file_path = None
    ...
    def Solve(self):
        if self.splitter is not None:
            subproblems = self.splitter.Split(self.clauses)
            if self.sorter is not None:
                for i, subproblem in enumerate(subproblems):
                    if len(self.sorter)==1:
                        self.subproblems.append(self.sorter[0].Sort(subproblem))
                    elif len(self.sorter)!=1 and len(subproblems)==len(self.sorter):
                        self.subproblems.append(self.sorter[i].Sort(subproblem))
                    else:
                        raise Exception("Length of subproblems generated by Splitter does not match with length of Sorter array.")    
                if self.gadget is not None:
                    ...
                    if self.joiner is not None:
                        if len(self.joiner)==1 and len(self.subproblems)==1:
                            self.Q, Q_encoding = self.joiner[0].Join(Mappers)
                        elif len(self.joiner)==1 and len(self.subproblems)!=1:
                            mappings = []
                            for Map in Mappers:
                                Q, Q_encode = self.joiner[0].Join([Map])
                        ...
                        self.Q = {k: v for k, v in self.Q.items() if (k[0] != k[1] and v != 0) or k[0] == k[1]} #Delete the zero-couplings
